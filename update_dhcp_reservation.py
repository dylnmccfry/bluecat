...
with open("ips.csv", 'r') as file:
    ...
    for row in reader:
        ...
        device_mac_list.append(row[1])
        device_hostname_list.append(row[2])


# Checks each IP address to see if it has been assigned and if it has valid data
for ip_address, mac_address, hostname in zip(device_ip_list, device_mac_list, device_hostname_list):
    ...
    if ip_to_update.get('id') == 0:
        print('This address is not currently in use.')
# This is where the script would set the reservation

# POST URL to update DHCP reservation with MAC address and hostname
        bam_post_url = f"http://{bam_url}/Services/REST/v1/assignIP4Address?action={action}&configurationId={config_id}&ip4Address={ip_address}&macAddress={mac_address}&properties=name={hostname}"
...
# This is where the script would delete the existing object, and then create the reservation.
        ...
        print(r.text)
        print('')
        print('Creating DHCP reservation...')
        bam_post_url = f"http://{bam_url}/Services/REST/v1/assignIP4Address?action={action}&configurationId={config_id}&ip4Address={ip_address}&macAddress={mac_address}&properties=name={hostname}"
        r = requests.post(bam_post_url, headers=headers)
        print(r)
        print(r.text)
